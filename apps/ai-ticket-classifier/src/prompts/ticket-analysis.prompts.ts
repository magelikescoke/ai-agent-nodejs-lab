import { TicketAnalysisCategories } from '../ticket/ticket-analysis.schema';
...
const ticketAnalysisPromptV2: TicketAnalysisPrompt = {
  ...
  systemPrompt: [
    ...
    'Do not add fields outside [category,overview,suggestedAction].',
    'Few-shot examples:',
    'Ticket: "I was charged after canceling my plan." Output: {"category":"billing","overview":"User reports an unexpected charge after cancellation.","suggestedAction":"Review cancellation and billing history, then refund if confirmed."}',
    'Ticket: "The API returns 500 for every export request." Output: {"category":"technical","overview":"User reports API failures during export requests.","suggestedAction":"Check API error logs and recent export service changes."}',
    'Ticket: "I lost access to my admin email and cannot sign in." Output: {"category":"account","overview":"User cannot access the admin account email.","suggestedAction":"Verify ownership and start the account recovery flow."}',
    'Ticket: "Your agent ignored my messages and closed the case." Output: {"category":"complaint","overview":"User is dissatisfied with prior support handling.","suggestedAction":"Escalate for support quality review and reopen the case if needed."}',
    'Ticket: "Do you provide a security questionnaire for vendor review?" Output: {"category":"other","overview":"User asks for vendor security documentation.","suggestedAction":"Share the security questionnaire or direct the user to the vendor documentation."}',
  ].join(' '),
  buildUserPrompt: buildTicketAnalysisUserPrompt,
};
